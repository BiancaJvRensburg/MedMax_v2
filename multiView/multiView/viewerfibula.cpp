...
void ViewerFibula::setPlaneOrientations(std::vector<Vec> mandPolyline, std::vector<Vec> axes){
    ...
    if(ghostPlanes.size()==0){
        ...
        Vec axis = Vec(0,0,1);
        leftPlane->rotatePlane(axis, alpha);      // in comments because this was never matched in the first place
...
void ViewerFibula::recieveTest(std::vector<Vec> axes){
    ...
    zPlaneAxis = leftPlane->getMeshVectorFromLocal(zPlaneAxis);     // convert to world coordinates
    Vec rotationAxis = rightPlane->getPosition() - leftPlane->getPosition();   // get the position in the fibula space (leave it in world coordinates)
    rotationAxis.normalize();
    ...
    std::cout << "Projection z : " << projZ.x << " " << projZ.y  << " " << projZ.z << std::endl;
    std::cout << "Projection g : " << projGoal.x << " " << projGoal.y  << " " << projGoal.z << std::endl;
    ...
    Vec x = axes[0];
    x = rightPlane->getMeshVectorFromLocal(x);
    x = leftPlane->getLocalVector(x);
    x.normalize();

    Vec y = axes[1];
    y = rightPlane->getMeshVectorFromLocal(y);
    y = leftPlane->getLocalVector(y);
    y.normalize();

    Vec z = axes[2];
    z = rightPlane->getMeshVectorFromLocal(z);
